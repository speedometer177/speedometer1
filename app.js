...
/* אחסון בטוח: בדפדפנים שחוסמים קבצי Cookie, עצם הגישה ל-localStorage זורקת
   SecurityError שמפילה את כל הקובץ (=דף לבן). העטיפה בודקת זמינות פעם אחת;
   כשאין אחסון — האתר עובד רגיל, פשוט בלי זיכרון מקומי (העדפות/קאש). */
...
function saveDeletedIds(){try{safeStorage.setItem('sp_deleted_ids',JSON.stringify([...deletedIds]));}catch(e){}}
...
async function prefetchTopBodies(){try{if(window.__bodiesPrefetched)return;window.__bodiesPrefetched=true;const need=articles.slice(0,6).filter(a=>!a.body||a.body.length===0).map(a=>a.id);if(need.length===0)return;const q='id,body,gallery,body_images';let rows;if(sbClient){const{data,error}=await sbClient.from('articles').select(q).in('id',need);if(error)throw error;rows=data;}else{const r=await fetchT(SB_URL+'/rest/v1/articles?select='+q+'&id=in.('+need.join(',')+')',{headers:{'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY}});if(!r.ok)return;rows=await r.json();}
if(!Array.isArray(rows))return;const byId={};rows.forEach(r=>{byId[r.id]=r;});articles.forEach(a=>{const h=byId[a.id];if(h){if(!a.body)a.body=h.body||'';if(!a.gallery)a.gallery=h.gallery||null;if(!a.bodyImages)a.bodyImages=h.body_images||null;}});}catch(e){}}
...
var my=null;try{my=safeStorage.getItem('sp_vote_'+aid);}catch(e){}
...
var prev=null;try{prev=safeStorage.getItem('sp_vote_'+aid);}catch(e){}
if(prev===vote)return;
...
try{safeStorage.setItem('sp_vote_'+aid,vote);}catch(e){}
...
/* קופסת CTA למחשבון שווי שימוש - מוצגת רק בכתבות רלוונטיות (קטגוריית קניית רכב, או תוכן שמזכיר ליסינג/רכב חברה/שווי שימוש). ריק בכל שאר הכתבות. */
function buildCalcCTA(a){var haystack=((a.title||'')+' '+(a.body||'')).toLowerCase();var keywords=['ליסינג','רכב חברה','רכב צמוד','שווי שימוש','תלוש שכר'];var isRelevant=a.cat==='buying'||keywords.some(function(k){return haystack.indexOf(k)!==-1;});if(!isRelevant)return'';return'<div style="margin:28px 0;padding:18px 20px;background:linear-gradient(120deg,#15181f,#1d222c);border-radius:14px;border-right:3px solid var(--red);display:flex;align-items:center;gap:14px;flex-wrap:wrap;"><div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(150deg,var(--red),var(--red-dark));display:flex;align-items:center;justify-content:center;flex-shrink:0;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><rect x="3" y="2" width="18" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><line x1="7" y1="11" x2="7" y2="11"/><line x1="12" y1="11" x2="12" y2="11"/><line x1="17" y1="11" x2="17" y2="11"/><line x1="7" y1="15" x2="7" y2="15"/><line x1="12" y1="15" x2="12" y2="15"/><line x1="17" y1="15" x2="17" y2="19"/><line x1="7" y1="19" x2="12" y2="19"/></svg></div><div style="flex:1;min-width:200px;color:#f3f4f6;font-size:0.93rem;font-weight:600;line-height:1.5;">רכב חברה או ליסינג? בדקו כמה עולה לכם <b style="color:#ff4d5e;">שווי השימוש</b> בחודש</div><a href="/usage-value-calculator.html" style="background:var(--red);color:#fff;font-weight:800;font-size:0.85rem;padding:10px 18px;border-radius:100px;text-decoration:none;white-space:nowrap;">למחשבון ←</a></div>';}
...
let galleryImages=[];async function handleGalleryUpload(input){const files=Array.from(input.files).slice(0,20-galleryImages.length);for(const file of files){const compressedFile=await compressImageFile(file,300);const src=await uploadImageOrBase64(compressedFile||null);if(src)galleryImages.push({src,caption:''});}
...
if(a.faq&&a.faq.length){a.faq.forEach((item,i)=>{const qEl=document.getElementById('faq-q-'+(i+1));const aEl=document.getElementById('faq-a-'+(i+1));if(qEl)qEl.value=item.q||'';if(aEl)aEl.value=item.a||'';});}
...
// מוסיף פסקת "עדכון חי" מתוארכת לכתבה קיימת (לסיפורים מתפתחים: ריקולים,
// תאונות עם חקירה, השקות עם עדכוני מחיר) — במקום לפרסם כתבה כפולה על
// אותו סיפור. מוצג בראש גוף הכתבה, החדש ביותר קודם, ומעדכן dateModified.
async function addLiveUpdate(id){
  const a=articles.find(x=>x.id===id);if(!a)return;
  const text=(prompt('טקסט העדכון (ייתוסף עם השעה הנוכחית בראש הכתבה):')||'').trim();
  if(!text)return;
  const now=new Date();
  const timeStr=now.toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'});
  const entry={time:timeStr,text:text,ts:now.toISOString()};
  const updates=(a.liveUpdates||[]).concat([entry]);
  let saved=false;
  if(sbClient){try{const{error}=await sbClient.from('articles').update({live_updates:updates}).eq('id',id);if(!error)saved=true;}catch(e){}}
  if(!saved){try{const _t=await getAuthToken();if(_t){const r=await fetchT(SB_URL+'/rest/v1/articles?id=eq.'+id,{method:'PATCH',headers:{'apikey':SB_KEY,'Authorization':'Bearer '+_t,'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({live_updates:updates})});if(r.ok)saved=true;}}catch(e){}}
  if(!saved){alert('שגיאה בשמירת העדכון לשרת — נסה שוב.');return;}
  a.liveUpdates=updates;saveLocal();
  alert('העדכון נוסף בהצלחה.');
  if(currentArticleId===id)openArticle(id,false);
}
...
function clearForm(){['a-title','a-sub','a-author','a-img','a-img-cap','a-score','a-body','a-flash-title','a-flash-text','a-read-time','a-tags','a-scheduled','yt-url-1','yt-url-2','yt-url-3','yt-url-4','body-img-1','body-img-cap-1','body-img-2','body-img-cap-2','body-img-3','body-img-cap-3','body-img-4','body-img-cap-4','body-img-5','body-img-cap-5','body-img-6','body-img-cap-6','body-img-7','body-img-cap-7','body-img-8','body-img-cap-8','body-img-9','body-img-cap-9','body-img-10','body-img-cap-10','spec-power','spec-torque','spec-zero','spec-range','spec-price','spec-model','spec-year','spec-country','spec-trims','spec-topspeed','spec-trunk','spec-consumption','spec-pros','spec-cons','score-driving','score-design','score-tech','score-comfort','score-value','faq-q-1','faq-a-1','faq-q-2','faq-a-2','faq-q-3','faq-a-3','faq-q-4','faq-a-4'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});const specSec=document.getElementById('specs-section');if(specSec)specSec.style.display='none';const faqSec=document.getElementById('faq-section');if(faqSec)faqSec.style.display='none';delete document.getElementById('a-title').dataset.editId;delete document.getElementById('a-title').dataset.srcUid;try{spFlashWordCount();}catch(e){}document.getElementById('a-cat').value='local';document.querySelectorAll('input[name="extra-cat"]').forEach(cb=>{cb.checked=false;const lbl=cb.closest('label');if(lbl){lbl.classList.remove('active');lbl.style.background='';lbl.style.color='';lbl.style.borderColor='var(--border)';}});galleryImages=[];renderGalleryPreview();const us=document.getElementById('upload-status');if(us)us.textContent='';const tabBtn=document.querySelectorAll('.admin-tab')[0];if(tabBtn)tabBtn.textContent='+ כתבה חדשה';}
...
try{safeStorage.setItem('sp_fontSize',size);}catch(e){}}
function toggleDyslexia(){document.body.classList.toggle('dyslexia-font');accTogglePill('btn-dyslexia');try{safeStorage.setItem('sp_dyslexia',document.body.classList.contains('dyslexia-font'));}catch(e){}}
function toggleContrast(){document.body.classList.remove('yellow-contrast');document.getElementById('btn-yellow')?.classList.remove('active');document.body.classList.toggle('high-contrast');accTogglePill('btn-contrast');try{safeStorage.setItem('sp_contrast',document.body.classList.contains('high-contrast'));}catch(e){}}
function toggleYellow(){document.body.classList.remove('high-contrast','dark-mode','grayscale');['btn-contrast','btn-dark-access','btn-gray'].forEach(id=>document.getElementById(id)?.classList.remove('active'));document.body.classList.toggle('yellow-contrast');accTogglePill('btn-yellow');const di=document.getElementById('dark-icon');if(di)di.innerHTML='';try{safeStorage.setItem('sp_yellow',document.body.classList.contains('yellow-contrast'));}catch(e){}}
function toggleGrayscale(){document.body.classList.toggle('grayscale');accTogglePill('btn-gray');try{safeStorage.setItem('sp_gray',document.body.classList.contains('grayscale'));}catch(e){}}
function toggleLinks(){document.body.classList.toggle('highlight-links');accTogglePill('btn-links');try{safeStorage.setItem('sp_links',document.body.classList.contains('highlight-links'));}catch(e){}}
function toggleHeadings(){document.body.classList.toggle('highlight-headings');accTogglePill('btn-headings');try{safeStorage.setItem('sp_headings',document.body.classList.contains('highlight-headings'));}catch(e){}}
function toggleAnimations(){document.body.classList.toggle('no-animations');accTogglePill('btn-anim');try{safeStorage.setItem('sp_noanim',document.body.classList.contains('no-animations'));}catch(e){}}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){['about-modal','access-modal','privacy-modal','terms-modal'].forEach(id=>closeModal(id));document.getElementById('access-overlay')?.classList.remove('open');document.getElementById('admin-login-overlay').style.display='none';}});document.addEventListener('click',function(e){const overlay=document.getElementById('access-overlay');const btn=document.querySelector('.access-btn');if(overlay.classList.contains('open')&&!overlay.contains(e.target)&&e.target!==btn){overlay.classList.remove('open');}});window.addEventListener('scroll',()=>{document.getElementById('back-top')?.classList.toggle('visible',window.scrollY>400);document.querySelector('header')?.classList.toggle('scrolled',window.scrollY>60);});let resizeTimer;let _lastViewportW=window.innerWidth;window.addEventListener('resize',()=>{if(window.innerWidth===_lastViewportW)return;_lastViewportW=window.innerWidth;clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{handleMobileHero();},200);});function restorePrefs(){try{const sun='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>';if(safeStorage.getItem('sp_dark')==='true'){document.body.classList.add('dark-mode');const di=document.getElementById('dark-icon');if(di)di.innerHTML=sun;document.getElementById('btn-dark-access')?.classList.add('active');const dl=document.getElementById('dark-label');if(dl)dl.textContent='מצב יום';}
if(safeStorage.getItem('sp_contrast')==='true'){document.body.classList.add('high-contrast');document.getElementById('btn-contrast')?.classList.add('active');}
if(safeStorage.getItem('sp_gray')==='true'){document.body.classList.add('grayscale');document.getElementById('btn-gray')?.classList.add('active');}
if(safeStorage.getItem('sp_yellow')==='true'){document.body.classList.add('yellow-contrast');document.getElementById('btn-yellow')?.classList.add('active');}
if(safeStorage.getItem('sp_dyslexia')==='true'){document.body.classList.add('dyslexia-font');document.getElementById('btn-dyslexia')?.classList.add('active');}
if(safeStorage.getItem('sp_links')==='true'){document.body.classList.add('highlight-links');document.getElementById('btn-links')?.classList.add('active');}
if(safeStorage.getItem('sp_headings')==='true'){document.body.classList.add('highlight-headings');document.getElementById('btn-headings')?.classList.add('active');}
if(safeStorage.getItem('sp_noanim')==='true'){document.body.classList.add('no-animations');document.getElementById('btn-anim')?.classList.add('active');}
const fs=safeStorage.getItem('sp_fontSize');setFontSize(fs||'normal');}catch(e){document.getElementById('fs-normal')?.classList.add('active');}}
...
function checkScheduledArticles(){const now=new Date();let changed=false;articles.forEach(a=>{if(a.scheduledAt&&new Date(a.scheduledAt)<=now){a.scheduledAt=null;a.date=now.toLocaleDateString('he-IL');a.time=now.toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'});changed=true;if(sbClient)sbClient.from('articles').update({scheduled_at:null,date:a.date,time:a.time}).eq('id',a.id).catch(()=>{});}});if(changed){saveLocal();buildHero();buildSections();buildTicker();buildBreakingTicker();renderAdminTable();}}
