...
function heroSrcset(url){if(typeof url!=='string')return '';if(url.indexOf('images.unsplash.com')!==-1){return [480,800,1200,1600].map(w=>url.replace(/([?&])w=\d+/,'$1w='+w)+' '+w+'w').join(', ');}if(isProxyable(url)){return [640,960,1400,2000].map(w=>wsrvHero(url,w)+' '+w+'w').join(', ');}return '';}
...
a.views=(a.views||0)+1;saveLocal();if(sbClient)Promise.resolve(sbClient.rpc('increment_article_views',{article_id:id})).catch(()=>{});document.getElementById('home-page').style.display='none';document.getElementById('article-page').style.display='block';document.getElementById('admin-page').style.display='none';updateMeta(a);const artCatEl=document.getElementById('art-cat');if(artCatEl){artCatEl.textContent=CAT_LABELS[a.cat];artCatEl.onclick=()=>{showHome();filterCat(a.cat);};artCatEl.style.cursor='pointer';artCatEl.title='לכל '+CAT_LABELS[a.cat];}
...
async function compressImage(file,maxKB=300){return new Promise(resolve=>{const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{let w=img.width,h=img.height;const MAX_DIM=1600;if(w>MAX_DIM||h>MAX_DIM){const ratio=Math.min(MAX_DIM/w,MAX_DIM/h);w=Math.round(w*ratio);h=Math.round(h*ratio);}
const canvas=document.createElement('canvas');canvas.width=w;canvas.height=h;const ctx=canvas.getContext('2d');ctx.drawImage(img,0,0,w,h);let quality=0.88;const tryCompress=()=>{canvas.toBlob(blob=>{if(!blob){resolve(null);return;}
